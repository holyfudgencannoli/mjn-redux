 ...
 * @param localUri location of the image required for saving
 * @param filename file name of the image
 * @returns URI string of the destination in which the image was saved
 */

export async function saveImage(localUri: string, filename: string): Promise<string> {
...
async function requestFilePermissions() {
  // On Android we rely on StorageAccessFramework.requestDirectoryPermissionsAsync,
  // which will show its own system picker/permission dialog as needed.
  // No separate MediaLibrary permission request is required here.
  return;
}


export async function saveReceiptWithSAF(tempFileUri: string, filename: string) {
  ...
  if (Platform.OS !== "android") {
    const iosPath = `${FileSystem.documentDirectory}mycologger/images/${filename}`;
    await FileSystem.writeAsStringAsync(iosPath, base64, {
      encoding: FileSystem.EncodingType.Base64,
    });
    return iosPath;
